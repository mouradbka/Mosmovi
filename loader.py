...
class CNNTweetDataset(Dataset):
    ...
    def __getitem__(self: Dataset, idx: int):
        chars = self.tweet_chars[idx]
        coords = torch.FloatTensor(self.coords[idx])

        if len(chars) > self.char_max_length:
            chars = chars[:self.char_max_length]
        elif 0 < len(chars) < self.char_max_length:
            chars = np.concatenate(
                (chars, np.zeros((self.char_max_length - len(chars), 256), dtype=np.float32)))
        elif len(chars) == 0:
            chars = np.zeros((self.char_max_length, 256), dtype=np.float32)
